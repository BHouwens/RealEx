...
Elm.Humanise = Elm.Humanise || {};
Elm.Humanise.make = function (_elm) {
   "use strict";
   _elm.Humanise = _elm.Humanise || {};
   if (_elm.Humanise.values) return _elm.Humanise.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.input,
              _U.list([$Html$Attributes.$class("input")
                      ,A3($Html$Events.on,"input",$Html$Events.targetValue,function (_p0) {    return A2($Signal.message,address,Update(_p0));})]),
              _U.list([]))
              ,A2($Html.p,_U.list([$Html$Attributes.$class("result")]),_U.list([$Html.text(model.human)]))]));
   });
   var NoOp = {ctor: "NoOp"};
   var initialModel = {human: "Type a regex to show its matchings",regex: ""};
   var Model = F2(function (a,b) {    return {human: a,regex: b};});
   var convert = function (regex) {    return A2($Basics._op["++"],regex,"blah");};
   var update = F2(function (action,model) {
      var _p1 = action;
      if (_p1.ctor === "Update") {
            return _U.update(model,{human: A2($Basics._op["++"],"Matches a ",convert(_p1._0))});
         } else {
            return model;
         }
   });
   return _elm.Humanise.values = {_op: _op,convert: convert,Model: Model,initialModel: initialModel,NoOp: NoOp,Update: Update,update: update,view: view};
};
...
Elm.Main.make = function (_elm) {
   ...
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   $Humanise = Elm.Humanise.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   ...
   var update = F2(function (action,model) {
      var _p0 = action;
      var updateHumaniseModel = A2($Humanise.update,_p0._0,model.humaniseModel);
      return _U.update(model,{humaniseModel: updateHumaniseModel});
   });
   var HumaniseAction = function (a) {    return {ctor: "HumaniseAction",_0: a};};
   var view = F2(function (address,model) {
      return A2($Html.div,_U.list([]),_U.list([A2($Humanise.view,A2($Signal.forwardTo,address,HumaniseAction),model.humaniseModel)]));
   });
   var initialModel = {humaniseModel: $Humanise.initialModel};
   var main = $StartApp$Simple.start({model: initialModel,view: view,update: update});
   var AppModel = function (a) {    return {humaniseModel: a};};
   return _elm.Main.values = {_op: _op,AppModel: AppModel,initialModel: initialModel,HumaniseAction: HumaniseAction,update: update,view: view,main: main};
};
